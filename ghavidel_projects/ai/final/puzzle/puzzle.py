...
class NumMatrixPuzzle:
    def __init__(self, matrix: np.ndarray, row_sums: np.ndarray, col_sums: np.ndarray, verbose: bool, unique: bool):
        ...
        # sum of remaining sum based on changing masks matrix
    ...
    def select_variable(self) -> Tuple[Optional[Tuple[int, int]], Optional[List[int]]]:
        min_options = np.inf
        candidates = []
        ...
        for row, col in self.variables:
            ...
            for mask in (1, 0):
                # check for uniqueness only for mask=1
                if self.unique and mask == 1:
                    row_vals = self.matrix[row, self.masks[row, :] == 1]
                    if val in row_vals:
                        continue

                    col_vals = self.matrix[self.masks[:, col] == 1, col]
                    if val in col_vals:
                        continue
    ...
    def check_uniqueness(self, row: int, col: int) -> bool:
        if self.unique:
            row_vals = self.matrix[row, self.masks[row, :] == 1]
            if len(row_vals) != len(np.unique(row_vals)):
                return False

            col_vals = self.matrix[self.masks[:, col] == 1, col]
            if len(col_vals) != len(np.unique(col_vals)):
                return False

        return True

    def forward_checking(self, var: Tuple[int, int]) -> bool:
        row, col = var

        # check for uniqueness
        if not self.check_uniqueness(row, col):
            return False
    ...
    def backtrack(self) -> bool:
        ...
        for mask in domain:
            ...
            if not self.forward_checking(var):
    ...
    def check_solution(self) -> bool:
        ...
        matrix = self.matrix * (self.masks == 1)
        ...
        return np.array_equal(np.sum(matrix, axis=1), self.row_sums) and \
            np.array_equal(np.sum(matrix, axis=0), self.col_sums)
...
def main():
    ...
    parser.add_argument('-f', default='puzzle.txt',
                        type=str, help='filepath to the puzzle, default: puzzle.txt')
    parser.add_argument('-o', default='solution.txt',
                        type=str, help='filepath to save result, default: solution.txt')
    parser.add_argument('-v', default=False,
                        action='store_true', help='verbose mode')
    parser.add_argument('-u', default=False,
                        action='store_true', help='unique solution')
    args = parser.parse_args()

    matrix, row_sums, col_sums = read_puzzle(args.f)

    puzzle = NumMatrixPuzzle(matrix, row_sums, col_sums,
                             verbose=args.v, unique=args.u)

    now = time.time()
    solution = puzzle.solve()

    if solution is not None:
        print(f'I found a matrix in {time.time() - now: .2f} seconds')

        print(solution)
        if puzzle.check_solution():
            write_puzzle(solution, args.o)

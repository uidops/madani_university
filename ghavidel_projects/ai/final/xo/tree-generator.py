...
def hierarchy_pos(graph, root, width=1.5, vert_gap=1, vert_loc=0, xcenter=0.5, pos=None, parent=None,):
    if pos is None:
        pos = {}
...
def build_game_tree_alpha_beta(state, graph, node_id, id_counter, depth=0, max_depth=6, alpha=-np.inf, beta=np.inf):
    global counter
    if depth > max_depth:
        return None
    ...
    for move in available_moves:
        ...
        if alpha >= beta:
            graph.add_node(
                child_id, label=None, current_player=current_player, pruned=True
            )

            graph.add_edge(node_id, child_id)
            counter += len(child_state.get_available_moves())
...
def main():
    ...
    draw_game_tree(graph, root_id=0)
